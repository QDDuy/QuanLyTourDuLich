...
if (isPost()) {
    ...
    } elseif ($filterAll['soluongnguoi'] < 0 || !isNumberInt($filterAll['soluongnguoi'])) {
        $errors['soluongnguoi']['require'] = "Số lượng người không hợp lệ";
    }
    ...
    } elseif ($filterAll['coc'] < 0 || !isNumberInt($filterAll['coc'])) {
        $errors['coc']['require'] = "Tiền cọc không được nhỏ hơn 0";
    }
    ...
    if (empty($errors)) {
        ...
        if ($insertStatus) {
            setFlashData('smg', 'Thêm đặt tour thành công!!');
        ...
        redirect('?modules=dattour&action=add');
    } else {
...
<div class="container">
    <div class="row" style="margin: 100px auto; ">
        <h2 class="text-center text-uppercase mb-4">Thêm đặt tour</h2>
        <?php
        if (!empty($smg)) {
            getSmg($smg, $smg_type);
        }
        ...
        <form action="" method="post">
            <div class="row ">
                <div class="col">
                    ...
                    <div class=" form-group mg-form">
                        ...
                        <input name="ngaydat" type="date" class="form-control" placeholder="Ngày đặt tour" value="<?php echo (!empty($old['ngaydat'])) ? date('d/m/Y', strtotime($old['ngaydat'])) : null ?>">
                        <?php
                        echo (!empty($errors['ngaydat'])) ?  '<span class="error">' . reset($errors['ngaydat']) . '</span>' : null;
                        ?>
                    </div>
                    <div class=" form-group mg-form">
            ...
            <a href="?modules=dattour&action=list"><button type="button" class="btn btn-secondary">Quay lại</button></a>
